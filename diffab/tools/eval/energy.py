...
from diffab.tools.ddg.predictor import predict_ddg
...
def eval_pred_ddg(task: EvalTask):
    wt_pdb = task.ref_path
    mut_pdb = task.in_path
    task.scores.update({'pred_ddg': predict_ddg(wt_pdb, mut_pdb)})
    return task
